...
async function resolveArtistId(ac, inputId) {
  ...
  // Chỉ gõ text, không click dropdown → tìm hoặc tạo
  if (typedName) {
    const found = allArtists.find(a => a.name.toLowerCase() === typedName.toLowerCase());
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Không thể tạo nghệ sĩ');
    allArtists.push({ id: data.id, name: typedName });
    return data.id;
  }
...
function initAdmin() {
  ...
  document.getElementById('songForm').addEventListener('submit', async e => {
    ...
    let artistId;
    try {
      artistId = await resolveArtistId(songArtistAC, 'songArtistInput');
    } catch (err) {
  ...
  document.getElementById('albumForm').addEventListener('submit', async e => {
    ...
    let artistId;
    try {
      artistId = await resolveArtistId(albumArtistAC, 'albumArtistInput');
    } catch (err) {
      showToast(err.message, 'error');
      btn.disabled = false;
      return;
...
async function saveEditSong() {
  ...
  try {
    if (editArtistAC?.getSelected()) {
      artistId = await resolveArtistId(editArtistAC, 'editArtistInput');
    } else {
